    ...
    if objects:                    # 检测到人脸时
        ...
        for s in range(1, NUM_SUBJECTS+1):
            ...
            for i in range(2, NUM_SUBJECTS_IMGS+1):
                ...
                dist += image.match_descriptor(d0, d1)  # Calculate the feature difference between the sample image and the detected face.
            print("Average dist for subject %d: %d"%(s, dist/NUM_SUBJECTS_IMGS))  # 打印与每个人的特征差异度
            pmin = min(pmin, dist/NUM_SUBJECTS_IMGS, s) # 特征差异度越小，被检测人脸与此样本更相似更匹配。

        print(name_list[num]) # num为当前最匹配的人的编号,然后从名字列表中取出对应人的名字。the number of the current best match
        if name_list[num] == '李亦然':
            pin.high()
        else:
            pin.low()
# 程序入口
